...
export const getAzureStorageConfig = async (token: string) => {
        let error = null;

        const res = await fetch(`${WEBUI_API_BASE_URL}/configs/storage/azure`, {
                method: 'GET',
                headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${token}`
...
export const setAzureStorageConfig = async (token: string, config: object) => {
        let error = null;
